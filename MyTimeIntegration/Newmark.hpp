...
public:
  ...
  //!
  //! @param J0, J1, J2 A functor that returns the Jacobians of the right hand
  //! side of
  ...
  Tstate DoStep(F f, DF J0, DF J1, DF J2, Tstate w0, Tstate v0, Tstate a0,
                Tstate initialGuess, double t0, double h) {
    ...
    auto ResidualFunction = [&](Tstate xNew) {
      auto aNew =
          1. / (beta * h) * ((xNew - x) / h - v) + (1. - 1. / (2. * beta)) * a;
      auto vNew = v + h * (1. - gamma / (2. * beta)) * a +
                  gamma / beta * ((xNew - x) / h - v);
      Tstate res;
      f(aNew, vNew, x, res, t);
      return (res);
    };
