...
/*  Solves Poisson equation on a Line:
 *  - d2u/dx2 = f
 *
...
int main(int argc, char *argv[]) {
  ...
  Group<NodeSimple> dirichletBoundary = {nd0, nd1};
  Group<NodeSimple> neumannBoundary = {};

  // ***********************************
  //   List of some possible solutions
  // ***********************************

  auto solution = [](double r) { return (1. - r) * r; };
  auto solutionDerivative = [](double r) { return (1. - 2. * r); };
  auto negativeLaplaceOfSolution = [](Eigen::VectorXd r) { return 2.; };

  auto elms = mesh.ElementsTotal();
  Tools::SetValues(elms, dof2, solution);
  ...
  GlobalDofVector loadVector = asmbl.BuildVector(
      ...
      [&](const CellIpData &cipd) {
        return pde.LoadVector(cipd, negativeLaplaceOfSolution);
      });
  ...
  for (auto &nd : neumannBoundary) {
    ...
    loadVector.J[dof1](nd.GetDofNumber(0)) +=
        normal * solutionDerivative(coord);
  }
  ...
  std::cout << stiffnessMxMod << std::endl;

  std::cout << "Force" << std::endl;
  std::cout << loadVectorMod << std::endl;
  ...
  visualize.WriteVtuFile("PoissonLine.vtu");
}
