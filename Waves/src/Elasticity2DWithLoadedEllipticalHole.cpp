...
/* Quarter of a Rectangular domain, scalar wave equation, symmetry conditions
 * Step function like force directed in x-direction opens crack
 */
int main(int argc, char *argv[]) {
  ...
  double holeX = 0.0; // ellipse main axis in X direction
...
Physical Surface("Domain") = {1};
  ...
  std::string resultDirectory = "/Elasticity2D_EllipticalCrack/";
  ...
  auto loadFunc = [&](const CellIpData &cipd, double tt) {
    ...
    double tractionMagnitude = 2. / tau * smearedStepFunction(tt, tau);
    // Eigen::Vector2d traction = cipd.GetJacobian().Normal() *
    // tractionMagnitude;
    // std::cout << tractionMagnitude << std::endl;
    Eigen::Vector2d traction(tractionMagnitude, 0.);
    loadLocal[dof1] = N.transpose() * traction;
    return loadLocal;
  };
  ...
  for (int i = 0; i < numSteps; i++) {
    ...
    if ((i * 100) % numSteps == 0) {
      ...
      visualizeResult(resultDirectoryFull.string() + "Crack_" +
